...
import { Search, Plus, X, User, Phone, Calendar, FileText, AlertTriangle, ChevronRight, Calculator } from "lucide-react";
import { supabase } from "./supabaseClient";
import { C, mono, serif, pill, btnPrimary, inputStyle, selectStyle, label, daysUntil, endOfServiceCalc } from "./tokens";
...
function DetailView({ e, onEdit, onDelete }) {
  ...
  const calc = endOfServiceCalc(Number(e.salary) || 0, e.contractStart);
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      <div style={{ marginTop: 20 }}>
        <button onClick={() => setShowCalc((s) => !s)} disabled={!calc} style={{ border: `1px solid ${C.line}`, background: C.paper, borderRadius: 8, padding: "10px 16px", fontSize: 13, fontFamily: mono, display: "flex", alignItems: "center", gap: 8, opacity: calc ? 1 : 0.5 }}>
          <Calculator size={14} /> {showCalc ? "Hide" : "Calculate"} end-of-service payout
        </button>
        {!calc && <div style={{ fontSize: 12, color: C.inkSoft, marginTop: 8 }}>Add a contract start date and salary to calculate this.</div>}

        {showCalc && calc && (
          <div style={{ background: C.paper, border: `1px solid ${C.line}`, borderRadius: 10, padding: 20, marginTop: 12 }}>
            <div style={{ fontSize: 12.5, color: C.inkSoft, marginBottom: 16, lineHeight: 1.6 }}>
              {calc.yearsOfService.toFixed(1)} years of service · daily wage KD {calc.dailyWage.toFixed(2)} ·
              full indemnity under Article 51: <b>KD {calc.fullIndemnity.toFixed(2)}</b>{calc.capped && " (capped at 1.5 years' wage)"}
            </div>

            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 200, background: C.bg, borderRadius: 8, padding: 16 }}>
                <div style={{ ...label, marginBottom: 8 }}>If terminated</div>
                <div style={{ fontFamily: mono, fontSize: 22, fontWeight: 600 }}>KD {calc.terminationPayout.toFixed(2)}</div>
                <div style={{ fontSize: 11.5, color: C.inkSoft, marginTop: 6, lineHeight: 1.5 }}>
                  Full amount — Kuwait law gives 100% of accrued indemnity when the employer ends the contract,
                  unless it's for gross misconduct grounds under Article 41 (in which case indemnity can be forfeited —
                  use judgment and ideally legal advice for that scenario, this app can't determine cause).
                </div>
              </div>
              <div style={{ flex: 1, minWidth: 200, background: C.bg, borderRadius: 8, padding: 16 }}>
                <div style={{ ...label, marginBottom: 8 }}>If resigns</div>
                <div style={{ fontFamily: mono, fontSize: 22, fontWeight: 600 }}>KD {calc.resignationPayout.toFixed(2)}</div>
                <div style={{ fontSize: 11.5, color: C.inkSoft, marginTop: 6, lineHeight: 1.5 }}>
                  {calc.resignationTierLabel} (Article 53).
                </div>
              </div>
            </div>
            <div style={{ fontSize: 11, color: C.inkSoft, marginTop: 14, lineHeight: 1.5 }}>
              This doesn't include unused annual leave payout (owed separately under Article 70) or notice pay.
              Estimate based on basic salary only — housing/transport allowances aren't included, matching how
              indemnity is calculated under Kuwait law. Not legal advice; confirm with PAM or a lawyer for anything disputed.
            </div>
          </div>
        )}
      </div>
