...
- **Manual attendance entry** — manager Overview → "Add attendance" lets you backfill a forgotten punch or correct a mistake for any employee/date; counts the same as a real punch for payroll
- **End-of-service payout calculator** — on any employee's record (Records tab), a "Calculate end-of-service payout" button shows both the termination and resignation figures side by side, per Kuwait Labor Law Articles 51 and 53
- Leave requests and balance figures are manager-only now — staff just see Punch in/out and their own leave request status, nothing else
...
## End-of-service payout calculator (Articles 51 & 53)

On any employee's record in the Records tab, there's a "Calculate
end-of-service payout" button that shows two figures side by side:

- **If terminated** — the full indemnity: 15 days' basic wage per year of
  service for the first 5 years, then a full month's wage per year after
  that, capped at 1.5 years' total wage (Article 51). Termination by the
  employer pays this in full, *unless* it's for gross misconduct grounds
  under Article 41 — the app can't determine cause, so use judgment (and
  ideally legal advice) if that's the situation.
- **If resigns** — the same formula, scaled down by years of service
  (Article 53): nothing under 3 years, half between 3–5 years, two-thirds
  between 5–10 years, and the full amount at 10+ years.

This doesn't include unused annual leave payout (owed separately under
Article 70) or notice pay — both are real amounts that can be owed on top
of this, just not automated here yet. As with the rest of the legal logic
in this app: a reasonable compliant structure based on the law's text, not
certified legal advice — confirm anything disputed with PAM or a lawyer.
